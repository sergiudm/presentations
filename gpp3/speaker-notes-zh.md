...
英文幻灯片，中文讲稿。共 11 页，建议节奏约 5 分 45 秒；Q&A 另按现场时间安排。
...
接下来先用一页说明 Harness 是什么，然后看两篇工作：DGM 怎样让编程 Agent 修改自己，Hyperagents 又怎样把改进过程本身变成可修改的程序。两部分都紧接着看实验结果。最后讨论如何区分 Harness 与模型的贡献，以及能否用强模型演化 Harness，再交给低成本模型运行。
...
## 08 · Future outlook（约 45 秒）

最后提出两个未来问题。第一，RSI 的能力增长有多少来自 Harness 的改变，又有多少依赖模型本身的能力，以及两者的相互作用？左边建议把初始和演化后的 Harness，分别搭配模型 A 和 B，在相同任务与预算下比较；还可以固定演化预算，改变元 Agent 所用的模型。第二，元 Agent 用强模型演化出来的 Harness，能否让另一个轻量、低成本的任务模型也受益？右边的想法是，先投入较高成本得到 H*，再固定它，只替换运行任务的模型。我们关心保留了多少效果、每个成功任务的成本是否下降，也要把前期演化成本摊进去。这些是进一步量化归因和迁移的研究设想，不是已经得到的结果。

## 09 · References（约 10 秒）
